 ...
 * <p/>
 * <b>IMPORTANT</b>: This is currently the only instance in the system in which
 * simultaneous connectivity to both the mobile data network and a Wi-Fi network
 * is allowed. This makes the code for handling network connectivity somewhat
 * different than it is in other applications. In particular, we want to be able
 * to send or receive MMS messages when a Wi-Fi connection is active (which
 * implies that there is no connection to the mobile data network). This has two
 * main consequences:
 * <ul>
 * <li>Testing for current network connectivity (
 * {@link android.net.NetworkInfo#isConnected()} is not sufficient. Instead, the
 * correct test is for network availability (
 * {@link android.net.NetworkInfo#isAvailable()}).</li>
 * <li>If the mobile data network is not in the connected state, but it is
 * available, we must initiate setup of the mobile data connection, and defer
 * handling the MMS transaction until the connection is established.</li>
 * </ul>
 */
				...
				try {
					...
					while (cursor.moveToNext()) {
						...
						case Transaction.RETRIEVE_TRANSACTION:
							...
							// downloading mode. If the user just turned on the
							// auto-retrieve
							// option, we also retry those messages that don't
							// have any errors.
							...
							if (!autoDownload) {
								// If autodownload is turned off, don't process
								// the
								// transaction.
							...
							// Logic is twisty. If there's no failure or the
							// failure
							// is a non-permanent failure, we want to process
							// the transaction.
							// Otherwise, break out and skip processing this
							// transaction.
		...
		try {
			synchronized (mProcessing) {
				...
				if (mPending.size() > 0) {
					...
					Message msg = mServiceHandler.obtainMessage(EVENT_HANDLE_NEXT_PENDING_TRANSACTION, transaction.getConnectionSettings());
					mServiceHandler.sendMessage(msg);
		...
		public void handleMessage(Message msg) {
			...
			case EVENT_TRANSACTION_REQUEST:
				...
				try {
					...
					Log.v(TAG, "EVENT_TRANSACTION_REQUEST MmscUrl=" + args.getMmscUrl() + " proxy port: " + args.getProxyAddress());
			...
			synchronized (mProcessing) {
				while (mPending.size() != 0) {
					...
					if (transaction instanceof SendTransaction) {
						...
						int respStatus = PduHeaders.RESPONSE_STATUS_ERROR_TRANSIENT_NETWORK_PROBLEM;
		...
		public void processPendingTransaction(Transaction transaction, TransactionSettings settings) {
			...
			if (transaction != null) {
				...
				try {
					...
					if (processTransaction(transaction)) {
						Log.v(TAG, "Started deferred processing of transaction  " + transaction);
					} else {
						transaction = null;
						stopSelf(serviceId);
					}
			...
			synchronized (mProcessing) {
				for (Transaction t : mPending) {
					if (t.isEquivalent(transaction)) {
						Log.v(TAG, "Transaction already pending: " + transaction.getServiceId());
						return true;
					}
				}
				...
				/*
				 * Make sure that the network connectivity necessary for MMS
				 * traffic is enabled. If it is not, we need to defer processing
				 * the transaction until connectivity is established.
				 */
				...
				if (connectivityResult == 1) {
					...
					Log.v(TAG, "processTransaction: connResult=APN_REQUEST_STARTED, " + "defer transaction pending MMS connectivity");
					return true;
				}
	...
	private class ConnectivityBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (mmsNetworkInfo == null) {
				...
				// This is a very specific fix to handle the case where the
				// phone receives an
				// incoming call during the time we're trying to setup the mms
				// connection.
				...
				if (mmsNetworkInfo.isConnected()) {
					TransactionSettings settings = new TransactionSettings(TransactionService.this, mmsNetworkInfo.getExtraInfo());
					// If this APN doesn't have an MMSC, mark everything as
					// failed and bail.
